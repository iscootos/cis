...
`#`开头的是编译预处理指令
####宏
`#define`用来定义一个宏，这个不是C语句，所以不用加分号结束           
```c
#define PI 3.14159
```
PI是宏的名称，3.14159是宏的值       
它把程序里面出现宏名称的地方，都替换为宏的值，也就是文本替换     
可以使用`gcc --save--temps`来查看临时文件中的`.i`文件      

如果一个宏的值中有其他宏的名字，也是会被替换的     

如果一个宏的值超过一行，最后一行之前的行末需要加`\`符号
...
#define _DEBUG
```
这类宏是用于条件编译的，然后有其他的预编译处理指令来检查这个宏是否已经被定义过了   
####预定义的宏
```c
__LINE__   //当前源代码的行号, 整数
__FILE__   //当前源代码的文件名， 字符串
__DATE__   //编译程序的日期，字符串
__TIME__   //编译程序的时间，字符串
__STDC__
```
####带参数的宏
像函数的宏，宏可以带参数
...
```
####标准头文件结构
头文件我们需要保证，只被引用一次，而不会被重复引用，那么我们就需要一个保护机制来判断是否已经被引用了 
```c
#ifndef __MAX_H__
#define __MAX_H__
