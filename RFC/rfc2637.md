 ...
 * Reserved0                 
   保留字段，此字段必须为0
 * Protocol Version                 
   协议版本，发送方请求的PPTP版本号和修订号。该字段4个字节，其中前两个字节为版本号，后两个字节为修订号。PPTP协议版本号默认版本号1，`0x0100`
 * Reserved1                 
   保留字段，此字段必须为0
 * Framing Capabilities                 
   成帧功能，发送方支持能够支持的帧类型。该字段长32位，但当前只定义了前两位。如果第1位被设置为1，则支持异步帧，如果第2位被设置为1，则支持同步帧。
```text
1 - Asynchronous Framing supported   异步帧支持
2 - Synchronous Framing supported    同步帧支持
```
 * Bearer Capabilities           
   载波功能，同样，该字段长32位，但当前只定义了前两位，第1位被设置为1表明支持模拟接入，第2位被设置为1表示支持数字接入。
```text
1 - Analog access supported      模拟访问支持
2 - Digital access supported     数字访问支持
```
 * Maximum Channels           
   最大信道数，在强制隧道模式，PAC使用该字段来指定在隧道中可支持多少个用户PPP会话，在自发隧道模式下，远程接入客户将该字段设置为0
 * Firmware Revision           
   固件修订号，指定远程接入客户PNS的固件修订号或软件驱动程序版本
 * Host Name           
   主机名，包含发送方的DNS名称。一个64字节的字段，包括发行的PPTP服务器或PPTP客户端的DNS名称,如果小于64字节长度，用0填充
 * Vendor Name           
   厂商字符串，包含厂商特定的信息，如远程客户PNS使用的软件类型。一个64字节的字段，包含他特定供应商字符串,如Microsoft如果小于64字节长度，用0填充
 ...
 * Result Code               
   结果编码字段长8位，用于指出5种结果状态，代码值如下：
```text
1 - Successful channel establishment    /* 成功建立了信道 */
2 - General error -- Error              /* 通用错误(由错误编码指出) */
3 - Command channel already exists      /* 控制信道已经存在 */
4 - Requester is not authorized to establish a command channel  /* 未授权请求方建立控制信道 */
5 - The protocol version of the requester is not supported      /* 不支持请求方指定的协议版本 */
```
 * Error Code         
...
一般错误代码
```text
0 (None) - No general error  /* 没有发送错误 */
1 (Not-Connected) - No control connection exists yet for this PAC-PNS pair  /* 没有该远程接入客户PNS-PAC的连接 */
2 (Bad-Format) - Length is wrong or Magic Cookie value is incorrect  /* 长度或Magic Cookie的值不正确 */
3 (Bad-Value) - One of the field values was out of range or reserved field was non-zero  /* 某个字段的值不在合法范围内或保留字段的值不为零 */
4 (No-Resource) - Insufficient resources to handle this command now  /* 当前没有足够的资源来处理该命令 */
5 (Bad-Call ID) - The Call ID is invalid in this context  /* 呼叫ID不合法 */
6 (PAC-Error) - A generic vendor-specific error occurred in the PAC  /* 厂商特定的错误(PAC上发生的错误) */
```
 ...
 * wait_ctl_reply                            
   当收到Start-Control-Connection-Reply，如果没有错误，发送方移动到建立状态，如果有错误，将发送方至移动到wait_stop_reply。            
 * established                            
   已经建立的链接，可以由任何一方终止，发送方，发送一个Stop-Control-Connection-Request请求，并进入wait_stop_reply状态,如果收到一个Stop-Control-Connection-Request，发送一个Stop-Control-Connection-Reply回复给发起方，并关闭TCP连接。            
 * wait_stop_reply                            
